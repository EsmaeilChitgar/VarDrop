...
def efficient_sampler(x, k, group_size, freq_list, min_thres=None, return_group=False, sampling='random'):
    # k-Dominant Frequency Hashing(k-DFH).
    if sampling == 'importance':
        hash_values, importance_scores = k_dominant_frequency_importance(
    ...
    for value in np.unique(hash_values):
        ...
        if sampling == 'importance':
            group_scores = importance_scores[group_indices]
            selected_order = np.argsort(-group_scores, kind='stable')
            group_indices = [group_indices[i] for i in selected_order[:min(group_size, len(group_indices))]]
        else:
            group_indices = np.random.choice(group_indices, min(group_size, len(group_indices)),
                                             replace=True)  # Any sampling method can be utilized.
...
def k_dominant_frequency_importance(batch_x, k, freq_list=None, min_thres=None):
    x_amp = torch.fft.rfft(batch_x, dim=1).abs()
    ...
    total_amp = torch.sum(x_amp, dim=0)
    total_amp = torch.clamp(total_amp, min=1e-8)

    importance_scores = torch.sum(k_amps, dim=0) / total_amp

    importance_scores = importance_scores.detach().cpu().numpy()

    return hash_values, importance_scores